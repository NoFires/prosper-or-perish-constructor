...
def _write_map_modes(path: Path, goods: Iterable[str]) -> None:
    ...
    blocks = [
        ...
        "# Purple-to-green classified diverging scale. Values are additive percentages (0.1 = +10%).",
        "# Wheat uses a harvest-neutral value; all modes mark matching raw-material locations with stripes.",
...
{_output_modifier_map_color_block(_productivity_value_name(good))}
\tsecondary_map_color = {{
\t\tif = {{
\t\t\tlimit = {{
\t\t\t\tis_ownable = yes
...
def _output_modifier_map_color_block(value_name: str) -> str:
    lines = [
        "\tmap_color = {",
        "\t\tif = {",
        "\t\t\tlimit = { is_ownable = no }",
    ...
    for (
        ...
        if condition is None:
            ...
            lines.extend(
                [
                    "\t\telse_if = {",
                    f"\t\t\tlimit = {{ {value_name} {condition} }}",
                ]
            )
        if width is None:
            lines.append(f"\t\t\tvalue = {_rgb(min_rgb)}")
        else:
            lines.extend(_output_modifier_bucket_lerp_lines(value_name, lower_bound, width, min_rgb, max_rgb))
        lines.append("\t\t}")
    lines.append("\t}")
    return "\n".join(lines)


def _output_modifier_bucket_lerp_lines(
    value_name: str,
    ...
    if lower_bound is None:
        raise ValueError("Gradient output modifier buckets require a lower bound")
    lines = [
        "\t\t\tlerp = {",
...
def _output_modifier_legend_keys(upper: str) -> str:
    lines = [
        ...
        for suffix, _description, _condition, _lower_bound, _width, _min_rgb, _max_rgb, legend_rgb in OUTPUT_MODIFIER_CLASSES
        if suffix in OUTPUT_MODIFIER_LEGEND_CLASSES
    ]
    lines.append(
...
def _write_script_values(
    path: Path,
    goods: Iterable[str],
    wheat_harvest_modifiers: list[tuple[str, str]],
) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    blocks = [
        "# Generated by scripts/generate_raw_material_local_map_modes.py - do not edit by hand.",
        "# Local output modifier values for the raw-material output map modes.",
        "",
    ]
    for good in goods:
        blocks.extend(_script_value_block(good, wheat_harvest_modifiers))
        blocks.append("")
    _write_crlf(path, "\n".join(blocks).rstrip() + "\n")


def _script_value_block(good: str, wheat_harvest_modifiers: list[tuple[str, str]]) -> list[str]:
    lines = [
...
def _localization_block(goods: Iterable[str]) -> str:
    lines = [
        LOCALIZATION_START,
        "  # Generated by scripts/generate_raw_material_local_map_modes.py - do not edit by hand.",
        '  PP_LOCAL_OUTPUT_MAPMODE_RANGE: "Purple-to-green productivity bands"',
    ]
    goods = list(goods)
    for good in goods:
...
def _output_modifier_legend_localization_lines(good: str) -> list[str]:
    ...
    if good == WHEAT_GOOD:
        ...
        description = (
            f"This colors [locations|e] by the local {display} output modifier. "
            f"Purple-to-green diverging bands separate low, neutral, and high productivity; stripes mark locations where {display} is the raw material."
        )
    lines = [f'  MAPMODE_PP_LOCAL_{upper}_OUTPUT_MODIFIER: "#T ${name_key}$#!\\n{description}"']
    lines.extend(
        f'  MAPMODE_PP_LOCAL_{upper}_OUTPUT_MODIFIER_{suffix}: "{_output_modifier_legend_description(good, suffix, class_description)}"'
        for suffix, class_description, _condition, _lower_bound, _width, _min_rgb, _max_rgb, _legend_rgb in OUTPUT_MODIFIER_CLASSES
        if suffix in OUTPUT_MODIFIER_LEGEND_CLASSES
    )
...
def _output_modifier_legend_description(good: str, suffix: str, description: str) -> str:
    if good == WHEAT_GOOD:
        return description
    display = _display_name(good).lower()
    generic_descriptions = {
        "EXTREME_DEFICIT": f"Extreme local {display} deficit",
        "MARGINAL": f"Marginal local {display} output",
        "NEUTRAL": f"Near neutral local {display} output",
        "STRONG": f"Strong local {display} output",
        "EXCEPTIONAL": f"Exceptional local {display} output",
    }
    return generic_descriptions.get(suffix, description.replace("wheat", display))
